...
TEST_CASE("merge handles every pass family") {
    ...
    src.fault_paged_payload.max_payload_bytes = 16384u;
    src.fault_paged_payload.page_size = 2048u;
    src.fault_paged_payload.delivery = "fault_paged";
    src.fault_paged_payload.backend = "lazy_accessor";
    src.fault_paged_payload.per_page_keys = true;
    src.fault_paged_payload.reseal_after_use = false;
    src.fault_paged_payload.decoy_pages = 3u;
    ...
    src.anti_dbg.allow_self_trace = false;
    ...
    CHECK(dst.fault_paged_payload.max_payload_bytes == 16384u);
    CHECK(dst.fault_paged_payload.page_size == 2048u);
    CHECK(dst.fault_paged_payload.delivery == "fault_paged");
    CHECK(dst.fault_paged_payload.backend == "lazy_accessor");
    CHECK(dst.fault_paged_payload.per_page_keys == true);
    CHECK(dst.fault_paged_payload.reseal_after_use == false);
    CHECK(dst.fault_paged_payload.decoy_pages == 3u);
    ...
    CHECK(dst.anti_dbg.allow_self_trace == false);
